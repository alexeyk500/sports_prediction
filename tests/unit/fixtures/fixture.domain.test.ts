...
import {
  ...
  type FixtureWithCompetition,
} from "@/modules/fixtures/fixture.domain";

const now = new Date("2026-09-05T12:00:00.000Z");

function fixture(
  overrides: Partial<FixtureWithCompetition> = {},
...
describe("fixture eligibility", () => {
  ...
  it("rejects non-open statuses", () => {
    for (const status of ["LOCKED", "LIVE"] as const) {
      expect(() =>
        assertFixtureEligibleForPrediction(fixture({ status }), now),
      ).toThrow(DomainError);
    }
  });
  ...
  it("rejects at or after kickoff", () => {
    ...
    expect(() =>
      assertFixtureEligibleForPrediction(fixture({ kickoffAt }), now),
    ).toThrow(/locked/);
    expect(() =>
      assertFixtureEligibleForPrediction(
        fixture({ kickoffAt }),
  ...
  it("rejects missing scoring snapshot", () => {
    expect(() =>
      assertFixtureEligibleForPrediction(
        fixture({ scoringSnapshotId: null }),
        now,
      ),
    ).toThrow(/published scoring snapshot/);
  });
