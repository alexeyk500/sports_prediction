...
export type SupportedCompetitionCode = (typeof SUPPORTED_COMPETITION_CODES)[number];

const supportedCompetitionCodes = new Set<string>(SUPPORTED_COMPETITION_CODES);

export interface FixtureWithCompetition
  extends Pick<Fixture, "id" | "status" | "kickoffAt" | "scoringSnapshotId"> {
...
export function assertFixtureEligibleForPrediction(
  ...
  if (fixture.status !== "OPEN") {
    throw new DomainError("FIXTURE_NOT_OPEN", "Fixture is not open for predictions.", {
      fixtureId: fixture.id,
  ...
  if (instant.getTime() >= fixture.kickoffAt.getTime()) {
    throw new DomainError("FIXTURE_LOCKED", "Fixture is locked for predictions.", {
      fixtureId: fixture.id,
