...
try {
  const report = JSON.parse(
    await readFile(FOOTBALL_ASSETS_DOWNLOAD_REPORT_PATH, "utf8"),
  );
  ...
  if (validation.issues.some((issue) => issue.severity === "error")) {
    console.error(
      "Cannot build canonical football assets manifest because validation failed.",
    );
  ...
  console.log(
    `Canonical football assets manifest written: ${FOOTBALL_ASSETS_MANIFEST_PATH}`,
  );
  ...
  const message =
    error instanceof Error
      ? error.message
      : "Unknown football assets manifest build error.";
